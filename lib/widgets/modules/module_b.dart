                ...
                if (students != null && students.containsKey(userId)) {
                  ...
                  moduleRef.orderByChild('title').equalTo('Week 1: Discovering Vowels').once().then((DatabaseEvent moduleEvent) {
                    ...
                    if (modules != null && modules.isNotEmpty) {
                      ...
                      final studentModuleRef = moduleRef.child('$moduleId/Module B');
      ...
      body: SingleChildScrollView(  // Wrap the entire body in a SingleChildScrollView
        ...
        child: Column(
          children: [
            ...
            WordCard(
              word: 'Blueberry',
              imageUrl: 'assets/images/blueberry.png', // Add blueberry image
              onMicPressed: () => _startListening('blueberry'),
              onSpeakerPressed: () => _playSound('blueberry'),
              isDisabled: _isDisabled['blueberry'] ?? false, // Safe access
            ),
            const Divider(), // Divider between fruits and other categories

            // Vehicles Category (Letter B)
            Text(
              'Vehicles',
              style: Theme.of(context).textTheme.titleLarge?.copyWith(
                fontSize: 24,
            ...
            WordCard(
              word: 'Bus',
              imageUrl: 'assets/images/bus.png', // Add bus image
              onMicPressed: () => _startListening('bus'),
              onSpeakerPressed: () => _playSound('bus'),
              isDisabled: _isDisabled['bus'] ?? false, // Safe access
            ),
            const Divider(), // Divider between vehicles and other categories

            // Animals Category (Letter B)
            Text(
              'Animals',
              style: Theme.of(context).textTheme.titleLarge?.copyWith(
                fontSize: 24,
            ...
            WordCard(
              ...
              onMicPressed: () => _startListening('butterfly'),
              onSpeakerPressed: () => _playSound('butterfly'),
              isDisabled: _isDisabled['butterfly'] ?? false, // Safe access
            ),
            const SizedBox(height: 16),
            WordCard(
              word: 'Bat',
              imageUrl: 'assets/images/bat.png', // Add bat image
              onMicPressed: () => _startListening('bat'),
              onSpeakerPressed: () => _playSound('bat'),
              isDisabled: _isDisabled['bat'] ?? false, // Safe access
            ),
            const Divider(), // Divider between animals and next content
          ],
        ),
      ),
    );
